    ...
    with torch.no_grad():
        for i, (input, mask, label) in tqdm(enumerate(val_loader)):
            ...
            if args.branch_S:
                ...
                output = F.softmax(pre_seg, dim=1)[:,1,:,:]
                result = np.uint8(255 * output.squeeze().cpu().numpy())
                result = cv2.applyColorMap(result, cv2.COLORMAP_JET)
                gt = np.uint8(255 * mask.squeeze().cpu().numpy())
                # cv2.imwrite('../result/Segprobability/'+str(i)+'_gt.png', gt)
                cv2.imwrite('../result/Segprobability/'+str(i)+'_pre.png', result)
