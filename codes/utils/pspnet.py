...
class PSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, seg_classes=2, cls_classes=2, zoom_factor=8,
        ...
        assert seg_classes > 1
        assert zoom_factor in [1, 2, 4, 8]
        self.zoom_factor = zoom_factor
        self.branchSeg = branchSeg
        self.ProbTo = ProbTo
        ...
        self.fc = nn.Linear(fea_dim//4, cls_classes)
        ...
        if branchSeg:
            ...
            self.cls = nn.Sequential(
                ...
                nn.Conv2d(512, seg_classes, kernel_size=1)
            )
        # if self.training:
...
if __name__ == '__main__':
    ...
    model = PSPNet(layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=21, zoom_factor=1, pretrained=True).cuda()
    model.eval()
    print(model)
    output = model(input)
